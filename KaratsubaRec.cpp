...
void karatsubaRec(int* x, int* y, int n, int** res)
{
	if (n == 1 || n == 0) {
		(*res)[1] += (x[0] * y[0]) % 10;
		(*res)[0] += (x[0] * y[0]) / 10;
	...
	shiftLeft(p1, *res, s1, 2 * n, n);//put p1 in correct place of c
	shiftLeft(p2, *res, s1, 2 * n, 0);//put p2 in correct place of c
	subtract(&p3, p1, s2, s1);//p3 -= p1
	subtract(&p3, p2, s2, s1);//(p3-p1) -= p2
	shiftLeft(p3, *res, s2, 2 * n, n / 2);//put p3 in correct place of c
	fixDigits(*res, 2 * n);//correct the array so each place has a single digit
